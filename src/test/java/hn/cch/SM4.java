...
public class SM4 {
     ...
     * @return
     */
    private long getLongBytes(byte[] b, int i) {
        long n = (long) (b[i] & 0xFF) << 24 | (long) ((b[i + 1] & 0xFF) << 16) | (long) ((b[i + 2] & 0xFF) << 8) | (long) (b[i + 3] & 0xFF);
        n = n & 0xFFFFFFFFL;
        return n;
    }

    private void setLongBytes(long n, byte[] b, int i) {
    ...
    public static final byte[] SboxTable = { // 4 * 64
    ...
    public static final int[] FK = { // 4 * 1
            0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc
    };

    public static final int[] CK = { // 4 * 32
    ...
    private long sm4Lt(long ka) {
        ...
        c = bb ^ ROL(bb, 2) ^ ROL(bb, 10) ^ ROL(bb, 18) ^ ROL(bb, 24);
        return c;
    }
    ...
    private void sm4_setkey(long[] SK, byte[] key) {
        ...
        MK[0] = getLongBytes(key, 0);
        MK[1] = getLongBytes(key, 4);
        MK[2] = getLongBytes(key, 8);
        MK[3] = getLongBytes(key, 12);
    ...
    private void sm4_one_round(long[] sk, byte[] input, byte[] output) {
        ...
        ulbuf[0] = getLongBytes(input, 0);
        ulbuf[1] = getLongBytes(input, 4);
        ulbuf[2] = getLongBytes(input, 8);
        ulbuf[3] = getLongBytes(input, 12);
        while (i < 32) {
            ulbuf[(i + 4)] = sm4F(ulbuf[i], ulbuf[(i + 1)], ulbuf[(i + 2)], ulbuf[(i + 3)], sk[i]);
            i++;
        }
        setLongBytes(ulbuf[35], output, 0);
        setLongBytes(ulbuf[34], output, 4);
        setLongBytes(ulbuf[33], output, 8);
        setLongBytes(ulbuf[32], output, 12);
    }

    private byte[] padding(byte[] input, boolean mode) {
        if (input == null) {
            return null;
        }
